...
def visualize_graph(
    data_dir: str, data_file: str, output_dir: str, output_format: typings.RenderFormat = "png"
) -> None:
    ...
    for node in graph["nodes"]:
        dot.node(node["submitter_id"], fillcolor=get_color(node["label"]), style="filled")

    for edge in graph["edges"]:
        dot.edge(edge["src"], edge["dst"])
    dot.render(view=True, directory=output_dir, format=output_format)
